  ...
  return (
    <section ref={sectionRef} className="relative bg-white py-20 sm:py-28 overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 text-balance">Why DocPrep?</h2>
        ...
        <div className="grid gap-6 sm:grid-cols-3 mb-12" role="list">
          {topFeatures.map((feature, index) => {
            ...
            return (
              <article
                key={index}
                role="listitem"
                className={`group rounded-2xl bg-white p-8 shadow-md hover:shadow-xl transition-all duration-500 border border-gray-100 hover:border-[#3D3D8F]/30 hover:-translate-y-2 ${
                  isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                }`}
                style={{ transitionDelay: `${delay}ms` }}
              >
                <div className="mb-6 inline-flex h-16 w-16 items-center justify-center rounded-full bg-blue-100 text-[#3D3D8F] group-hover:bg-[#3D3D8F] group-hover:text-white transition-all duration-300 group-hover:scale-110" aria-hidden="true">
        ...
        <div className="grid gap-6 sm:grid-cols-3" role="list" aria-label="Platform statistics">
          {stats.map((stat, index) => {
            ...
            return (
              <article
                key={index}
                role="listitem"
                className={`group relative overflow-hidden rounded-2xl bg-gradient-to-br ${stat.gradient} p-8 shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 hover:scale-105 ${
                  isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                }`}
                style={{ transitionDelay: `${delay}ms` }}
              >
                ...
                <div className="relative">
                  <div className="mb-4 text-5xl sm:text-6xl font-bold text-white">{stat.number}</div>
                  <h3 className="mb-2 text-xl font-bold text-white">{stat.title}</h3>
